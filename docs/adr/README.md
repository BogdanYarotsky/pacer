...
| **Structure** | `0028` Rows owns row order · `0029` Display owns drawn strings · `0030` Layout and CandleMath are pure · `0031` annotate leaves, not renderers · `0032` the version is debug-only · `0033` no computed numbers in prose · `0034` a sideload cannot be verified from the host · `0035` the jungle sourcePath is literal |
